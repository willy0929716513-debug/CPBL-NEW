...
- **投手數據（pitching）**：跟打者共用同一個網址（`/stats/recordall`），
  ...
  （不是單純的「投手」），已改用
  `scraper/http.py` 的 `get_rendered_html_after_selecting()`
  （Playwright 實際在頁面上切換分頁後再讀取渲染結果）並修正選項文字。
  投手分頁實際的表頭欄位還沒對照過真實資料確認，`pitching.py` 目前列的
  欄位名稱是延用打者頁面的慣例猜測，可能還需要再對一次。
- **賽程與戰報（schedule）**：確認官網賽程頁是 Vue.js 單頁應用（SPA），
  比賽卡片是瀏覽器執行 JavaScript 後才動態產生。已對照真實卡片結構修正：
  比賽狀態是外層卡片自己的 class（目前只確認過 "final"＝已完賽，其他狀態
  值還沒見過真實範例）、主客隊與比分都是同一個 class 前綴（`team`/`num`）
  加上 away/home 修飾字，不是獨立欄位名稱。**比賽日期目前還沒找到卡片
  以外的日期標題怎麼取得**（很可能是「一個日期標題底下放好幾張當天的卡片」
  這種結構），程式會嘗試往前找最近的日期類元素，找不到就留空、不影響其他
  欄位。
