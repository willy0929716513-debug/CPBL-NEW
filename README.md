...
- **投手數據（pitching）**：跟打者共用同一個網址（`/stats/recordall`），
  官網用前端 Vue 元件切換分頁、不會改變網址，一般的 HTTP 請求永遠只會拿到
  預設的打者分頁。下拉選單裡「投手」這個選項實際顯示文字是「投手成績」
  （不是單純的「投手」）。**目前遇到的狀況是：即使 Playwright 確實選取了
  這個選項，抓回來的表格內容仍然跟打者分頁一模一樣**——很可能這種查詢頁面
  選完選項後還需要額外按「查詢/搜尋」按鈕才會真的重新查詢，
  `get_rendered_html_after_selecting()` 現在會在偵測到「切換後內容看起來
  沒變」時，自動多嘗試點擊查詢/搜尋/送出按鈕一次；如果這樣還是不行，
  代表這個下拉選單可能根本不是控制資料的開關，需要看下一次的錯誤訊息
  （這次會附上完整渲染後的頁面內容）才能判斷下一步。
- **賽程與戰報（schedule）**：✅ 已對照官網真實結構修正並確認可用（球隊、
  比分、狀態、球場都能正確解析）。比賽狀態是外層卡片自己的 class（目前
  只確認過 "final"＝已完賽，其他狀態值還沒見過真實範例）、主客隊與比分
  都是同一個 class 前綴（`team`/`num`）加上 away/home 修飾字。**比賽日期
  目前還沒找到卡片以外的日期標題怎麼取得**（很可能是「一個日期標題底下
  放好幾張當天的卡片」這種結構），程式會嘗試往前找最近的日期類元素，
  找不到就留空、不影響其他
  欄位。
