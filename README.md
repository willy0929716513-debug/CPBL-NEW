...
- **投手數據（pitching）**：跟打者共用同一個網址（`/stats/recordall`），
  官網用前端 Vue 元件切換「打者/投手/守備」分頁、不會改變網址，一般的
  HTTP 請求永遠只會拿到預設的打者分頁。已改用
  `scraper/http.py` 的 `get_rendered_html_after_selecting()`
  （Playwright 實際在頁面上切換到「投手」分頁後再讀取渲染結果），但投手
  分頁實際的表頭欄位還沒對照過真實資料確認，`pitching.py` 目前列的欄位
  名稱是延用打者頁面的慣例猜測，可能還需要再對一次。
- **賽程與戰報（schedule）**：確認官網賽程頁是 Vue.js 單頁應用（SPA），
  比賽卡片是瀏覽器執行 JavaScript 後才動態產生，一般的 HTTP 請求拿到的
  HTML 只有篩選用的下拉選單、沒有任何比賽資料。因此改用
  `scraper/http.py` 的 `get_rendered_html()`（Playwright + headless
  Chromium 實際渲染頁面）取得內容，但目前卡片的 CSS class 名稱還沒確認，
  GAME_CARD_SELECTOR 等常數可能還需要再調整一次。
